...
void main() {
    ...
    //displacement
    float displacement = (displacementPattern * 0.1);
    vec3 newPosition = position + (normal * displacement);// newpos = oldpos + (direction * noise)
    // mvp 
    ...
    // varying 
    vPosition = position; // assign position
    vNormal = normal;
    vUv = uv;
    vDisplacement = displacementPattern;
}